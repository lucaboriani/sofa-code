...
describe('decide', () => {
  ...
  it('mounts when in viewport + idle', () => {
    expect(decide({ ...base, inViewport: true })).toBe('mount');
  });
  ...
  it('does not mount when already running', () => {
    expect(decide({ ...base, inViewport: true, currentState: 'running' })).toBe('noop');
  });

  it('tears down when running and scrolls out of viewport', () => {
    expect(decide({ ...base, inViewport: false, currentState: 'running' })).toBe('teardown');
  });

  it('tears down when running and reducedMotion becomes true', () => {
    expect(decide({ ...base, inViewport: true, reducedMotion: true, currentState: 'running' })).toBe('teardown');
  });
});
