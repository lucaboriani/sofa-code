...
export const createAudio = (ctx: AudioContext): RoomAudio => {
  ...
  // Bus expects a node; we use a silent gain so it has something to disconnect.
  const silent = ctx.createGain();
  (silent as GainNode).gain.value = 0;
...
export const mount: RoomMount = (canvas, opts) => {
  ...
  const loop = createRafLoop((_dtMs, tMs) => {
    ...
    void frameCollisions; // available for future audio integration
